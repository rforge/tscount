loglin.sim <- function(n, param=list(intercept=1, past_obs=NULL, past_mean=NULL, xreg=NULL), model=list(past_obs=NULL, past_mean=NULL, xreg=NULL, external=FALSE), distr=c("poisson", "nbinom"), distrcoefs, fit, n_start=50){
  ...
  result <- list(ts=z, mean=kappa, xreg_effect=xreg_effect)
  return(result)
}
