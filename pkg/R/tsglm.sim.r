tsglm.sim <- function(n, param=list(intercept=1, past_obs=NULL, past_mean=NULL, xreg=NULL), model=list(past_obs=NULL, past_mean=NULL, xreg=NULL, external=FALSE), link=c("identity", "log"), distr=c("poisson", "nbinom"), distrcoefs, fit, n_start=50){
  ...
  if(!missing(fit)){
    ...
    if(!missing(distr) && !missing(distrcoefs)){
      warning("Conditional distribution provided in argument 'distr' with additional distribution coefficients provided in argument 'distrcoefs' is used, the distribution in argument 'fit$distr' and its additional distribution coefficients in argument 'fit$distrcoefs' is ignored")
    }else{
      distr <- fit$distr
...
#    if(missing(link)) stop("Argument 'link' is missing. Either provide this argument or an argument 'fit' with the result of a previous model fit")
#    if(missing(distr)) stop("Argument 'distr' is missing. Either provide this argument or an argument 'fit' with the result of a previous model fit")
#    if(missing(distrcoefs)) stop("Argument 'distrcoefs' is missing. Either provide this argument or an argument 'fit' with the result of a previous model fit")
  }
